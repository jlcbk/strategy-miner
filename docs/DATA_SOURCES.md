...
`instrument-assumption` 可在人工审核后写入显式静态元数据假设：

- 必须提供 `evidence_hash` 和 `assumption_note`。
- source 固定为 `manual_instrument_metadata_assumption`。
- 只能用于研究 data lake 的元数据覆盖，不代表官方历史 exchangeInfo。
- 写入前应有对应的 `metadata_resolution_plan` 说明可接受来源、拒绝条件和人工审核边界。

## Fee Schedule

Fee schedule 用统一 `EventType.FEE` 和 `FeeSchedule` payload 表达。当前先接入显式手工假设，不把它标记为交易所官方账户费率：

- `fee-assumption`：
  - 写入 `maker_bps`、`taker_bps` 和 `tier`。
  - 默认命令规划使用 `maker_bps=10`、`taker_bps=10`、`tier=conservative_manual`。
  - source 固定为 `manual_fee_assumption`。

注意：真实手续费依赖交易所、产品、VIP 等级、maker/taker、活动优惠和账户状态。`fee-assumption` 只用于回放和验证时显式扣费；进入人工审核前应替换为真实费率表或账户级费率来源。
